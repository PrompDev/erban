...
$task = "OpenClaw Gateway (erban)"
$port = 18901
$idPort = 8766

Write-Host "=== Erban uninstall ($(if($dry){'dry run - pass -Yes to act'}else{'LIVE'})) ==="

# 1. Stop + delete the erban gateway scheduled task (NOT the main one)
...
foreach ($pn in @($port, $idPort)) {
  $procs = Get-CimInstance Win32_Process -Filter "Name='node.exe'" | Where-Object { $_.CommandLine -match "$pn" -or $_.CommandLine -match 'openclaw-erban' -or $_.CommandLine -match 'identity-service' }
...
# 3. Remove erban profile/state, runtime cache, and the vendored gate copy
